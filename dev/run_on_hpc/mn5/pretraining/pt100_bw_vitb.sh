...
#SBATCH --gres=gpu:4
#SBATCH --output=%j_pt100_bw_vitb.out
#SBATCH --error=%j_pt100_bw_vitb.err
#SBATCH --chdir=.
...
CONFIG_NAME="${CONFIG_NAME:-bal_bw_vitb}"
...
print_run_header
echo "--- Executing I-JEPA (blockwise masking) ---"
singularity exec --nv \
    --bind "$BIND_ARGS" \
    "$SIF_IMAGE" \
