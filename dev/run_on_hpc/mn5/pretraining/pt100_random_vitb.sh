...
#SBATCH --gres=gpu:4
#SBATCH --output=%j_pt100_random_vitb.out
#SBATCH --error=%j_pt100_random_vitb.err
#SBATCH --chdir=.
...
print_run_header
echo "--- Executing I-JEPA (random masking) ---"
singularity exec --nv \
    --bind "$BIND_ARGS" \
    "$SIF_IMAGE" \
