...
#SBATCH --gres=gpu:4
#SBATCH --output=%j_lp_mn40_vitL_green.out
#SBATCH --error=%j_lp_mn40_vitL_green.err
#SBATCH --chdir=.
...
RUN_ID="${SLURM_JOB_ID:-manual}_lp_mn_vitb_4gpu"
...
print_run_header
echo "--- Executing Distributed Linear Probe (4 GPUs, mn checkpoint) ---"
singularity exec --nv \
    --bind "$BIND_ARGS" \
    "$SIF_IMAGE" \
