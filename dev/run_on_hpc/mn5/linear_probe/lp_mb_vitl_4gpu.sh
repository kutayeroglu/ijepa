...
#SBATCH --gres=gpu:4
#SBATCH --output=%j_lp_mb_vitl.out
#SBATCH --error=%j_lp_mb_vitl.err
#SBATCH --chdir=.
...
# -- Config --
CONFIG_TAG="${CONFIG_TAG:-bal_mb_vitl}"
PRETRAINING_RUN_ID="${PRETRAINING_RUN_ID:-37744670_bal_mb_vitl}"
CHECKPOINT_SUFFIX="${CHECKPOINT_SUFFIX:-latest}"
RUN_ID="${SLURM_JOB_ID:-manual}_lp100_mb_vitl_4gpu"
...
print_run_header
echo "--- Executing Distributed Multiblock ViT-L Linear Probe (4 GPUs) ---"
singularity exec --nv \
    --bind "$BIND_ARGS" \
    "$SIF_IMAGE" \
