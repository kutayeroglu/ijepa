...
# -- Config (blockwise pretrain: bal_bw_vitb) --
CONFIG_TAG="${CONFIG_TAG:-bal_bw_vitb}"
# TODO: set to your pretraining run folder name, e.g. 12345678_bal_bw_vitb
PRETRAINING_RUN_ID="${PRETRAINING_RUN_ID:-00000000_bal_bw_vitb}"
CHECKPOINT_SUFFIX="${CHECKPOINT_SUFFIX:-latest}"
RUN_ID="${SLURM_JOB_ID:-manual}_lp100_bw_4gpu"
...
print_run_header
echo "--- Executing Distributed Linear Probe (4 GPUs, blockwise checkpoint) ---"
singularity exec --nv \
    --bind "$BIND_ARGS" \
    "$SIF_IMAGE" \
